...
from ficus.models.models import TradeDirection
...
class Vantage(ITradingCallback):
    ...
    async def prepare_listeners(self, symbols_to_subscribe: List[str]):
        try:
            ...
            logger.error(f'Failed on preparing the metaapi listeners {traceback.format_exc()}')
    ...
    async def open_trade(self, symbol: str, direction: int,
        ...
        if direction == TradeDirection.BUY:
            return await (self.__api_connection
                          .create_market_buy_order(symbol=symbol, volume=volume))
        elif direction == TradeDirection.SELL:
            return await (self.__api_connection
                          .create_market_sell_order(symbol=symbol, volume=volume))
    ...
    async def close_trade(self, trade: FicusTrade, trading_symbol) -> MetatraderTradeResponse:
        try:
            ...
            logger.info(f"Failed to close position for {trade}: {traceback.format_exc()}")
    ...
    async def partially_close_trade(self, trade: FicusTrade, symbol: str) -> MetatraderTradeResponse:
        try:
            ...
            logger.info(f"Failed to partially close position for {trade}: {traceback.format_exc()}")
    ...
    async def modify_trade(self, trade: FicusTrade) -> MetatraderTradeResponse:
        try:
            ...
            logger.info(f"Failed to modify trade {trade}: {traceback.format_exc()}")
