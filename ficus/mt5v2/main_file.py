...
from metaapi_cloud_sdk.metaapi.metaapi import MetaApiOpts
...
async def meta_api_synchronization():
    options_london = MetaApiOpts(region='london')
    api = MetaApi(Vantage.META_API_TOKEN, options_london)
    try:
        ...
        #rpc
        candles = await account.get_historical_candles(symbol='BTCUSD', timeframe='1m',
                                                       start_time=datetime.fromisoformat('2024-06-03'), limit=60*24)
        print(candles)
