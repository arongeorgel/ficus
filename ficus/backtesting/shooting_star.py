...
def candlestick(df, ax=None, highlight=None, titlename='',
                ...
                colorup='r', colordown='backtesting', highlightcolor='y',
                **kwargs):
    # bar width
    # use 0.6 by default
...
def plot(data, name):
    ...
    ax2.plot(data.loc[data['signals'] == 1].index,
             data['Close'].loc[data['signals'] == 1],
             marker='^', lw=0, c='backtesting', label='long',
             markersize=10)

    # only show five tickers
    plt.xticks(
        range(0, len(data), len(data)),
        data['Date'][0::len(data)].dt.date
    )
...
def main():
    ...
    df = yfinance.download(ticker, start=stdate, end=eddate, interval='1h')
